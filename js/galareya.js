class Filter {
    ...
    getDiv() {
        const main_div = document.querySelectorAll(this.selector);
...
class Gallery {
    ...
    constructor(data = {}) {
        ...
        this.images = {};
        this.images_n = 0;
        this.gallery_div_full_scrin = 'gallery_div_full_scrin';
        if (data.duration)
            this.default_duration = data.duration;
        else
    ...
    openImage(e) {
        ...
        if (element.hasAttribute('data-full')) {
            src = element.getAttribute('data-full');
        } else {
    ...
    buildGallery() {
        ...
        div.setAttribute('id', this.gallery_div_full_scrin);
        ...
        document.getElementById(this.gallery_div_full_scrin).addEventListener("click", galleryClick);
        document.getElementById('gallery_close').addEventListener("click", this.destroyGallery);
        document.getElementById('gallery_close').addEventListener("click", this.destroyGallery.bind(this));

    }

    destroyGallery() {
        if (document.getElementById(this.gallery_div_full_scrin)) {
            document.getElementById(this.gallery_div_full_scrin).remove();
        }
    ...
    changeImage(i) {
        ...
        let div = document.getElementById(this.gallery_div_full_scrin);
    ...
    drow(timePassed) {
        const div = document.getElementById(this.gallery_div_full_scrin);
        if (div) {
            const img = div.querySelectorAll('img');
            const coefficient = this.linear(timePassed / this.duration);
            const opacity_0 = 1 - coefficient;
            const opacity_1 = 0 + coefficient;
...
function galleryInit() {
    ...
    gallery.gallery();
    const div = gallery.getDiv();
    for (let i = 0; i < div.length; i++) {
        div[i].getElementsByTagName('img')[0].addEventListener("click", imageClick);
    }


    const target = document.querySelector(gallery.selector);
